...
func TestResolveBindings_FieldFillsPathParam(t *testing.T) {
	...
	resolved, remaining, err := resolveBindings(mr, &apiFlags{}, fields)
...
func TestResolveBindings_NonMatchingFieldPassesThrough(t *testing.T) {
	...
	_, remaining, err := resolveBindings(mr, &apiFlags{}, fields)
	require.NoError(t, err)
...
// context-kind param (orgName/projectName/stackName) — provided the
// corresponding context flag is not also set.
...
// TestResolveBindings_ContextFlagAndFieldConflict verifies that supplying
// BOTH --org and -F orgName on an endpoint with {orgName} produces a
// structured invalid-flags error.
func TestResolveBindings_ContextFlagAndFieldConflict(t *testing.T) {
	t.Parallel()
	mr := &MatchResult{
		Op: &Operation{Method: "GET", Path: "/api/orgs/{orgName}"},
		Bindings: map[string]Binding{
	...
	_, _, err := resolveBindings(mr, &apiFlags{org: "from-flag"}, fields)
	require.Error(t, err)
	var apiErr *APIError
...
func TestResolveBindings_MissingParamErrorSuggestsFlag(t *testing.T) {
	...
	_, _, err := resolveBindings(mr, &apiFlags{}, nil)
	require.Error(t, err)
	var apiErr *APIError
	require.True(t, errors.As(err, &apiErr))
...
func TestResolveBindings_NullFieldRejected(t *testing.T) {
	...
	_, _, err := resolveBindings(mr, &apiFlags{}, fields)
	require.Error(t, err)
	var apiErr *APIError
	require.True(t, errors.As(err, &apiErr))
	assert.Equal(t, ErrInvalidFlags, apiErr.Envelope.Error.Code)
...
func TestResolveBindings_FieldValueStringification(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			resolved, _, err := resolveBindings(mr, &apiFlags{}, fields)
			require.NoError(t, err)
...
// TestAPIClientDo_ForwardsUserHeaders pins the contract that -H/--header
// values make it onto the outgoing HTTP request. Authorization is reserved
// (can't be overridden by a user header); everything else is forwarded and
// wins over the encoder's defaults (Accept, Content-Type).
func TestAPIClientDo_ForwardsUserHeaders(t *testing.T) {
	t.Parallel()
	...
	client := NewAPIClient(srv.URL, "my-token")
	hdrs := []ParsedHeader{
	...
	resp, err := client.Do(t.Context(), http.MethodGet, "/echo", nil, nil,
		"application/json", "application/json", hdrs)
	require.NoError(t, err)
	resp.Body.Close()
...
func TestValidateFlagCombos_BodyAndInputMutuallyExclusive(t *testing.T) {
	t.Parallel()
	err := validateFlagCombos(&apiFlags{
...
func TestEncodeFields_BodyFlagPassesThroughVerbatim(t *testing.T) {
	...
	body, extras, ct, err := encodeFields(
		"POST", fields, &apiFlags{body: `{"name":"acme"}`}, &Operation{},
	)
	require.NoError(t, err)
...
func TestEmitDryRun_Envelope(t *testing.T) {
	t.Run("valid_json_body_inlined", func(t *testing.T) {
		out := captureStdout(t, func() {
			err := emitDryRun("POST", "https://example.test/api/x?y=1",
	...
	t.Run("non_json_body_quoted", func(t *testing.T) {
		out := captureStdout(t, func() {
			err := emitDryRun("POST", "https://example.test/x", nil,
...
func TestDumpRequestVerbose_DropsUserAuthorization(t *testing.T) {
	stderr := captureStderr(t, func() {
		dumpRequestVerbose("GET", "https://example.test/api/x",
			[]ParsedHeader{
				{Name: "Authorization", Value: "token attacker"},
...
// rendering, --silent suppression, --include status+headers, --jq transform
// including the non-JSON fallback warning, and 4xx returning an APIError.
//
//nolint:paralleltest // mutates os.Stdout/os.Stderr
func TestHandleResponse(t *testing.T) {
	...
	t.Run("200_renders_body", func(t *testing.T) {
		out := captureStdout(t, func() {
			require.NoError(t, handleResponse(newResp(200, "application/json", `{"a":1}`), &apiFlags{}))
		})
		assert.Contains(t, out, `"a"`)
	})
	t.Run("204_empty", func(t *testing.T) {
		out := captureStdout(t, func() {
			require.NoError(t, handleResponse(newResp(204, "application/json", ""), &apiFlags{}))
		})
		assert.Empty(t, strings.TrimSpace(out))
	})
	t.Run("silent_suppresses_body", func(t *testing.T) {
		out := captureStdout(t, func() {
			require.NoError(t, handleResponse(newResp(200, "application/json", `{"a":1}`),
				&apiFlags{silent: true}))
		})
		assert.Empty(t, strings.TrimSpace(out))
	})
	t.Run("include_prints_status_line_and_headers", func(t *testing.T) {
		out := captureStdout(t, func() {
			require.NoError(t, handleResponse(newResp(200, "application/json", `{}`),
				&apiFlags{include: true}))
		})
		assert.Contains(t, out, "HTTP/1.1")
		assert.Contains(t, out, "Content-Type: application/json")
	})
	t.Run("jq_filters_json_body", func(t *testing.T) {
		out := captureStdout(t, func() {
			require.NoError(t, handleResponse(newResp(200, "application/json", `{"name":"acme"}`),
				&apiFlags{jq: ".name"}))
		})
		assert.Equal(t, "acme\n", out)
	})
	t.Run("jq_on_non_json_warns_and_passes_through", func(t *testing.T) {
		var out string
		stderr := captureStderr(t, func() {
			out = captureStdout(t, func() {
				require.NoError(t, handleResponse(newResp(200, "text/plain", `hello`),
					&apiFlags{jq: "."}))
			})
		})
		assert.Contains(t, stderr, "warning: --jq on non-JSON")
		assert.Equal(t, "hello", out)
	})
	t.Run("4xx_returns_apierror", func(t *testing.T) {
		err := handleResponse(newResp(404, "application/json", `{"code":404}`), &apiFlags{})
		require.Error(t, err)
		var apiErr *APIError
		require.True(t, errors.As(err, &apiErr))
	...
	t.Run("401_returns_auth_apierror", func(t *testing.T) {
		err := handleResponse(newResp(401, "application/json", `{"code":401}`), &apiFlags{})
		require.Error(t, err)
		var apiErr *APIError
		require.True(t, errors.As(err, &apiErr))
